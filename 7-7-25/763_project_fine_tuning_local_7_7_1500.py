# -*- coding: utf-8 -*-
"""763_Project_Fine_Tuning_Local.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1ihhkKCoiiYBNIKyNO74tK8oQy5MJq2Qu
...
q3 = df_imputed['SalePrice'].quantile(0.75)
iqr = q3 - q1
lower = 20000  # <<< BUSINESS RULE
upper = 300000 # <<< BUSINESS RULE
...
# Now X_val, y_val are for internal evaluation. "X_val" is reserved for final, unlabeled data.
X_train, X_val, y_train, y_val = train_test_split(X_final, y, test_size=0.2, random_state=42, stratify=y)
...
cm = confusion_matrix(y_val_clean, y_pred_clean, labels=labels_str)
...
print("y_val_clean unique:", sorted(y_val_clean.unique()))
print("labels_str:", labels_str)

# Get all unique values (in order of appearance in test set)
labels_str = list(pd.Series(y_val_clean).unique())
print("Using these labels for confusion matrix:", labels_str)

print("First 10 y_val values:", y_val[:10])
print("First 10 y_val_ord values:", y_val_ord[:10])
...
cm = confusion_matrix(y_val_ord, y_pred_ord, labels=labels)
...
pca = PCA(n_components=2)
X_pca_test = pca.fit_transform(X_val)
...
"""## Added to test on training set results
# --- Clean and prepare test set exactly as train ---
...
encoded_cats_test = encoder.transform(df_test[cat_cols_test])
combined_test = np.hstack((df_test[numeric_cols_test], encoded_cats_test))
imputed_test = imputer.transform(combined_test)
...
decoded_cats_test = pd.DataFrame(encoder.inverse_transform(imputed_cats_test), columns=cat_cols_test)
df_test_clean = pd.concat([imputed_numeric_test, decoded_cats_test], axis=1)

X_val = df_test_clean.drop(columns=['SalePrice', 'Id'], errors='ignore')
X_val = pd.get_dummies(X_val, drop_first=True)
# Align columns (add missing features as zeros)
for col in X_final.columns:
    if col not in X_val.columns:
        X_val[col] = 0
X_val = X_val[X_final.columns]
X_val[numeric_feats] = scaler.transform(X_val[numeric_feats])

# --- Predict price bins for test set ---
test_pred_bins = final_model.predict(X_val)
df_test['PredictedPriceCategory'] = test_pred_bins

print(df_test[['Id', 'PredictedPriceCategory']].head())
...
# Ensure y_val and y_pred are pandas Series with categories
# import pandas as pd
...
"""### Misclassification Distance by True Class
- x axis the "True Bin" (true class price range for each misclassified sample.
- y axis the difference between predicted bin index and true bin index (Predicted - True)
"""
...
y_val_ord_aligned = y_val_ord.reset_index(drop=True)
y_pred_ord_aligned = y_pred_ord.reset_index(drop=True) # Ensure y_pred_ord is also aligned

# Map bins to integer indices for error calculation using the aligned Series
y_true_idx = y_val_ord_aligned.cat.codes
y_pred_idx = y_pred_ord_aligned.cat.codes
...
# Use the aligned y_val_ord for comparison
is_misclassified = y_val_ord_aligned != y_pred_ord_aligned
...
df_error = pd.DataFrame({
    'True Bin': y_val_ord_aligned[is_misclassified], # Use aligned Series
    'Distance': misclassified_error
})

import matplotlib.pyplot as plt

# Count the number of rows in each class in the misclassified set
counts = df_error['True Bin'].value_counts().sort_index()

fig, axs = plt.subplots(2, 1, figsize=(12, 10), gridspec_kw={'height_ratios': [3, 1]})

# --- Boxplot ---
df_error.boxplot(column='Distance', by='True Bin', grid=False, ax=axs[0])
axs[0].axhline(0, color='gray', linestyle='--', lw=1)
axs[0].set_title('Misclassification Distance by True Class')
axs[0].set_xlabel('')
axs[0].set_ylabel('Predicted Bin - True Bin')
axs[0].tick_params(axis='x', rotation=45)
axs[0].figure.suptitle('')

# --- Bar plot ---
counts.plot(kind='bar', ax=axs[1], color='skyblue')
axs[1].set_title('Number of Misclassified Rows per True Bin')
axs[1].set_xlabel('True Bin')
...
result = permutation_importance(final_model, X_val, y_val, n_repeats=10, random_state=42)
# Align importances with the features that were actually used in the permutation importance calculation
importances = pd.Series(result.importances_mean, index=X_val.columns) # Create a pandas Series
